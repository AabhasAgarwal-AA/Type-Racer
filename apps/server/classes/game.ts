    ...
    setupListeners(socket: Socket){
        socket.on('start-game', async () => {
            if(this.gameStatus === "in-progress"){
                return socket.emit("error", "The game has alredy started");
            }
            if(this.gameHost !== socket.id){
                return socket.emit("error", "You are not the host of the game, only the host of the game can start the game");
            }
